...
impl<'a> FormulaFnTypeLowering<'a> {
        ...
        for arg in args {
            match arg {
                ...
                syn::FnArg::Typed(pt) => {
                    ...
                    if self.is_bare_closure_type_param(ty) {
                        model_inputs.push(syn::parse_quote!(#pat: #lowered_ty));
                    } else {
                        model_inputs.push(
                            syn::parse_quote!(#pat: <#lowered_ty as thrust_models::Model>::Ty),
                        );
                    }
                }
            }
    ...
    fn is_bare_closure_type_param(&self, ty: &syn::Type) -> bool {
