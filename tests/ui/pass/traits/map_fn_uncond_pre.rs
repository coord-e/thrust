...
where <I as thrust_models::Model>::Ty: PartialEq
{
    ...
    fn invariant(self) -> bool {
        // self.iter.invariant() &&
        // forall(|i: I::Item| pre!(self.func(i)))
        // With `Fn`, the closure's precondition is a fixed fact about the item alone
        // (no mutable state to track), so it is stated unconditionally instead of
        // "for every item the inner iterator could still step to" -- that
        // reachability-conditioned form is what made the `FnMut` invariant
        // non-inductive.
        "(and
