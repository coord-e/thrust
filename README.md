...
- `THRUST_SOLVER_TIMEOUT_SECS`: Timeout for waiting on results from the solver. A solver that exceeds it is sent `SIGTERM` so that it can release what it holds, and is killed if it has not exited two seconds later. Default: `30`
