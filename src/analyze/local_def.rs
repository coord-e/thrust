...
impl<'tcx, 'ctx> Analyzer<'tcx, 'ctx> {
    ...
    fn refine_basic_blocks(&mut self) {
        ...
        for (bb, _data) in mir::traversal::postorder(&self.body) {
            ...
            if bb == mir::START_BLOCK {
                let live_set: HashSet<Local> = live_locals.iter().map(|(l, _)| *l).collect();
                live_locals = self
                    .body
                    .args_iter()
                    .map(|a| {
                        let decl = &self.body.local_decls[a];
                        (a, TypeAndMut { ty: decl.ty, mutbl: decl.mutability })
                    })
                    .collect();
                for a in self.body.args_iter() {
                    if !live_set.contains(&a) {
                        self.drop_points
                            .get_mut(&bb)
                            .unwrap()
                            .before_statements
    ...
    fn assert_entry(&mut self, expected: &rty::RefinedType) {
        ...
        let entry_ty = entry_ty.to_function_ty();
