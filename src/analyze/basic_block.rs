...
use rustc_middle::mir::{self, BasicBlock, Body, Local, Operand, Place, Rvalue, TerminatorKind};
...
struct ReborrowVisitor<'a, 'tcx, 'ctx> {
    tcx: TyCtxt<'tcx>,
    analyzer: &'a mut Analyzer<'tcx, 'ctx>,
}
...
impl<'tcx, 'ctx> Analyzer<'tcx, 'ctx> {
    ...
    fn reborrow_visitor<'a>(&'a mut self) -> ReborrowVisitor<'a, 'tcx, 'ctx> {
        ReborrowVisitor {
            tcx: self.tcx,
            analyzer: self,
        }
